...
/*   Updated: 2023/10/12 04:50:20 by gmachado         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
			...
			t_trace_args *t)
{
	...
	get_point_light(&t->light, &(t_vec3){.x = 0, .y = 0, .z = 10},
...
void	trace_all(t_args *args, t_trace_args *t)
{
	...
	set_cone(&t->obj, matrix_multiply(rotate_z, transform),
		&t->phong.obj->material);
	set_object_limits(&t->obj, -0.7, 0.7, TRUE);
	y = 0;
