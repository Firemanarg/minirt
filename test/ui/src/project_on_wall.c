...
/*   Updated: 2023/09/21 11:01:21 by gmachado         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	trace_pixel(t_args *args, t_trace_args *t, int x, int y)
{
	if (hit(t->intersections))
		ft_pixel_put(&args->mlx_data, x, y, convert_color(&(t->obj.color)));
	else
...
void	trace_all(t_args *args, t_trace_args *trace_args)
{
	...
	set_sphere(&trace_args->obj, matrix_new_identity(4), &color);
	y = 0;
